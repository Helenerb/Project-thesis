...
comp.aPc.1 = ~ -1 + 
  ...
  rho0(x, model = "rw2", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior.1, scale.model = TRUE) + 
  rho1(x, model = "rw2", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior.1, scale.model = TRUE) + 
  phi(t, model = "rw2", values = unique(stomach.cancer$t), constr = TRUE, hyper = pc.prior.1, scale.model = TRUE) + 
  psi0(k, model = "rw2", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior.1, scale.model = TRUE) + 
  psi1(k, model = "rw2", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior.1, scale.model = TRUE) + 
  epsilon(xts, model = "iid", hyper = pc.prior.1)

# using prior 2
comp.aPc.2 = ~ -1 + 
  mu(s, model = "iid", hyper = list(prec = list(fixed = TRUE, initial = log(0.001)))) +
  rho0(x, model = "rw2", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior.2, scale.model = TRUE) + 
  rho1(x, model = "rw2", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior.2, scale.model = TRUE) + 
  phi(t, model = "rw2", values = unique(stomach.cancer$t), constr = TRUE, hyper = pc.prior.2, scale.model = TRUE) + 
  psi0(k, model = "rw2", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior.2, scale.model = TRUE) + 
  psi1(k, model = "rw2", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior.2, scale.model = TRUE) + 
  epsilon(xts, model = "iid", hyper = pc.prior.2)

# using prior 3
comp.aPc.3 = ~ -1 + 
  mu(s, model = "iid", hyper = list(prec = list(fixed = TRUE, initial = log(0.001)))) +
  rho0(x, model = "rw2", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior.3, scale.model = TRUE) + 
  rho1(x, model = "rw2", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior.3, scale.model = TRUE) + 
  phi(t, model = "rw2", values = unique(stomach.cancer$t), constr = TRUE, hyper = pc.prior.3, scale.model = TRUE) + 
  psi0(k, model = "rw2", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior.3, scale.model = TRUE) + 
  psi1(k, model = "rw2", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior.3, scale.model = TRUE) + 
...
gg.pred <- ggplot(data.pred %>%
  ...
  labs(title = "Sensitivity analysis on precision priors - aPc model, stomach cancer", x = "Age groups", y = "Mortality rate") + 
  facet_wrap(~year)
gg.pred

ggsave('sensitivity-analysis-aPcby-age-stomach.png',
       plot = gg.pred,
       device = "png",
...
gg.pred.cohort <- ggplot(data.pred %>%
  ...
  scale_fill_manual(name = "Prior",
                    values = palette.basis) +
  scale_shape_manual(values = c(3,2)) + 
  labs(title = "Sensitivity analysis on precision priors - aPc model, stomach cancer", x = "Cohort", y = "Mortality rate") + 
  facet_wrap(~year)

gg.pred.cohort

ggsave('sensitivity-analysis-aPc-by-cohort-stomach.png',
       plot = gg.pred.cohort,
       device = "png",
       path = '/Users/helen/OneDrive - NTNU/Vår 2021/Project-thesis/real-data/real-data-multivariate/Figures',
       height = 5, width = 8, 
...
gg.pred.period <- ggplot(data.pred %>%
  ...
  scale_fill_manual(name = "Prior",
                    values = palette.basis) +
  scale_shape_manual(values = c(3,2)) + 
  labs(title = "Sensitivity analysis on precision priors - aPc model, stomach cancer", x = "Year", y = "Mortality rate") + 
...
ggsave('sensitivity-analysis-aPc-by-period-stomach.png',
       plot = gg.pred.period,
       device = "png",
       path = '/Users/helen/OneDrive - NTNU/Vår 2021/Project-thesis/real-data/real-data-multivariate/Figures',
       height = 5, width = 8, 

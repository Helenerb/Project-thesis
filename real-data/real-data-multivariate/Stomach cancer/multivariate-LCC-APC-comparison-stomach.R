...
# Format data to more easily compare the two model performances:
data.pred <- rbind(data.pred.abKg, data.pred.aPc) %>%
  mutate("method" = rep(c("LCC : common period", "APC: common period"), each = 648))

# calculate score statistics and compare
...
gg.pred <- ggplot(data.pred %>%
  ...
  labs(title = "LCC and APC compared - Stomach cancer data", x = "Age groups", y = "Mortality rate") + 
  facet_wrap(~year)
gg.pred

ggsave('multivariate-comparison-by-age-stomach.png',
       plot = gg.pred,
       device = "png",
...
gg.pred.cohort <- ggplot(data.pred %>%
  ...
  scale_fill_manual(name = "Prediction method",
                    values = palette.basis) +
  scale_shape_manual(values = c(3,2)) + 
  labs(title = "LCC and APC compared - stomach cancer", x = "Cohort", y = "Mortality rate") + 
  facet_wrap(~year)

gg.pred.cohort

ggsave('multivariate-comparison-by-cohort-stomach.png',
       plot = gg.pred.cohort,
       device = "png",
       path = '/Users/helen/OneDrive - NTNU/Vår 2021/Project-thesis/real-data/real-data-multivariate/Figures',
       height = 5, width = 8, 
...
gg.pred.period <- ggplot(data.pred %>%
  ...
  scale_fill_manual(name = "Prediction method",
                    values = palette.basis) +
  scale_shape_manual(values = c(3,2)) + 
  labs(title = "LCC and APC compared - stomach cancer", x = "Year", y = "Mortality rate") + 
...
ggsave('multivariate-comparison-by-period-stomach.png',
       plot = gg.pred.period,
       device = "png",
       path = '/Users/helen/OneDrive - NTNU/Vår 2021/Project-thesis/real-data/real-data-multivariate/Figures',
       height = 5, width = 8, 

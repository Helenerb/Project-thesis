...
A.mat = matrix(1, nrow = 1, ncol = length(unique(stomach.cancer$x))) 
e.vec = 1

# common age effect: ABkg
comp.ABkg = ~ -1 +
  mu(s, model = "iid", hyper = list(prec = list(fixed = TRUE, initial = log(0.001)))) +
  alpha(x, model = "rw1", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  ...
  kappa0(t.c, model = "rw1", values = unique(stomach.cancer$t), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  kappa1(t.c, model = "rw1", values = unique(stomach.cancer$t), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  gamma0(k, model = "rw1", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  gamma1(k, model = "rw1", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
...
likelihood.ABkg.0 = like(formula = form.ABkg.0, family = "poisson", data = stomach.cancer.until2007.0, E = stomach.cancer.until2007.0$population)

form.ABkg.1 = cases ~ -1 + mu + alpha + beta*phi1 + beta*kappa1 + gamma1 + epsilon
likelihood.ABkg.1 = like(formula = form.ABkg.1, family = "poisson", data = stomach.cancer.until2007.1, E = stomach.cancer.until2007.1$population)
...
comp.abKg = ~ -1 +
  mu(s, model = "iid", hyper = list(prec = list(fixed = TRUE, initial = log(0.001)))) +
  alpha0(x, model = "rw1", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  alpha1(x, model = "rw1", values = unique(stomach.cancer$x), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  ...
  kappa(t.c, model = "rw1", values = unique(stomach.cancer$t), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  gamma0(k, model = "rw1", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  gamma1(k, model = "rw1", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  epsilon(xts, model = "iid", hyper = pc.prior)
...
comp.abkG = ~ -1 +
  ...
  kappa0(t.c, model = "rw1", values = unique(stomach.cancer$t), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  kappa1(t.c, model = "rw1", values = unique(stomach.cancer$t), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  gamma(k, model = "rw1", values = unique(stomach.cancer$k), constr = TRUE, hyper = pc.prior, scale.model = TRUE) +
  epsilon(xts, model = "iid", hyper = pc.prior) 
...
likelihood.ABKG = like(formula = form.ABKG, family = "poisson", data = stomach.cancer.until2007, E = stomach.cancer.until2007$population)
...
gg.pred <- ggplot(data.pred %>%
  ...
  labs(title = "Multivariate LCC models - stomach cancer", x = "Age groups", y = "Mortality rate") + 
  facet_wrap(~year)
gg.pred

ggsave('multivariate-LCC-by-age-stomach.png',
       plot = gg.pred,
       device = "png",
...
gg.pred.cohort <- ggplot(data.pred %>%
  ...
  scale_fill_manual(name = "Prediction method",
                    values = palette.basis) +
  scale_shape_manual(values = c(3,2)) + 
  labs(title = "Multivariate LCC models - stomach cancer", x = "Cohort", y = "Mortality rate") + 
  facet_wrap(~year)

gg.pred.cohort

ggsave('multivariate-LCC-by-cohort-stomach.png',
       plot = gg.pred.cohort,
       device = "png",
       path = '/Users/helen/OneDrive - NTNU/Vår 2021/Project-thesis/real-data/real-data-multivariate/Figures',
       height = 5, width = 8, 
...
gg.pred.period <- ggplot(data.pred %>%
  ...
  scale_fill_manual(name = "Prediction method",
                    values = palette.basis) +
  scale_shape_manual(values = c(3,2)) + 
  labs(title = "Multivariate LCC models - stomach cancer", x = "Year", y = "Mortality rate") + 
...
ggsave('multivariate-LCC-by-period-stomach.png',
       plot = gg.pred.period,
       device = "png",
       path = '/Users/helen/OneDrive - NTNU/Vår 2021/Project-thesis/real-data/real-data-multivariate/Figures',
       height = 5, width = 8, 
...
ggsave('effects-LCC-no-common-stomach.png',
       plot = p.abkg,
       device = "png",
       path = '/Users/helen/OneDrive - NTNU/Vår 2021/Project-thesis/real-data/real-data-multivariate/Figures',
       height = 5, width = 8, 
...
ggsave('effects-LCC-common-period-stomach.png',
       plot = p.abKg,
       device = "png",
       path = '/Users/helen/OneDrive - NTNU/Vår 2021/Project-thesis/real-data/real-data-multivariate/Figures',
       height = 5, width = 8, 

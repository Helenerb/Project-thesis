# Comparison of the LC and APC models that previously seem to give the best results
# in prediction of 2015 and 2016. 
...
pc.prior.epsilon <- list(prec = list(prior = "pc.prec", param = c(0.00929, 0.5)))
...
form.lc = total ~ -1 + Int + alpha + beta*phi + gamma + epsilon

likelihood.lc.l = like(formula = form.lc, family = "poisson", data = lung.cancer.wo1516, E = lung.cancer.wo1516$total.t)
likelihood.lc.s = like(formula = form.lc, family = "poisson", data = stomach.cancer.wo1516, E = stomach.cancer.wo1516$total.t)
...
res.apc.l = bru(components = comp.apc,
                  likelihood.apc.l, 
...
res.apc.l = bru_rerun(res.apc.l)

res.apc.s = bru(components = comp.apc,
                  likelihood.apc.s, 
                  options = list(verbose = F,
                                 bru_verbose = 1, 
                                 num.threads = "1:1",
...
data.pred.l <- rbind(data.apc.l, data.lc.l) %>%
  mutate("method" = rep(c("APC", "LC"), each = 324))

data.pred.s <- rbind(data.apc.s, data.lc.s) %>%
  mutate("method" = rep(c("APC", "LC"), each = 324))

gg.age.facet.l <- ggplot(data = data.pred.l, aes(x = year)) + 
  geom_vline(xintercept = "2014", color = palette.light[5]) + 
  geom_errorbar(aes(min = `0.025quant`, ymax = `0.975quant`, color = `method`), alpha = 0.5) +
  #geom_ribbon(aes(x = year, ymin = `0.025quant`, ymax = `0.975quant`, color = `method`, fill = `method`), alpha = 0.3) + 
  geom_point(aes(y = `mortality rate`, color = "Observed", shape = "Observed")) +
  geom_point(aes(y = mean, color = `method`, shape = `method`), size = 0.5) + 
  scale_color_manual(name = "Prediction method",
                     values = palette.basis) +
  scale_shape_manual(name = "Prediction method", values = c(1,1,4)) + 
...
gg.age.facet.s <- ggplot(data = data.pred.s, aes(x = year)) + 
  geom_vline(xintercept = "2014", color = palette.light[5]) + 
  geom_errorbar(aes(min = `0.025quant`, ymax = `0.975quant`, color = `method`), alpha = 0.5) +
  #geom_ribbon(aes(x = year, ymin = `0.025quant`, ymax = `0.975quant`, color = `method`, fill = `method`), alpha = 0.3) + 
  geom_point(aes(y = `mortality rate`, color = "Observed", shape = "Observed")) +
...
gg.pred.2016.l <- ggplot(data.pred.l %>% filter(year == "2016"), aes(x = fct_inorder(age))) + 
  ...
  geom_point(aes(y = mean, color = `method`, shape = `method`)) + 
  geom_point(aes(y = `mortality rate`, color = "Observed", shape = "Observed")) + 
  scale_color_manual(name = "Prediction method",
                     values = palette.basis) +
  scale_shape_manual(name = "Prediction method", values = c(16,16,4)) + 
  scale_x_discrete(guide = guide_axis(check.overlap = TRUE)) + 

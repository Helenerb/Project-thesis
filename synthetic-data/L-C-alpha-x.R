...
p.prec.beta.2.1 <- ggplot(data.frame(res.2.1$marginals.hyperpar) %>%
  ...
  geom_vline(data = res.2.1$summary.hyperpar, aes(xintercept = mean[2], color = "Mean", fill = "Mean")) + 
  geom_vline(data = res.2.1$summary.hyperpar, aes(xintercept = mode[2], color = "Mode", fill = "Mode")) + 
...
p.prec.epsilon.2.1 <- ggplot(data.frame(res.2.1$marginals.hyperpar) %>%
  ...
  geom_vline(data = res.2.1$summary.hyperpar, aes(xintercept = mean[4], color = "Mean", fill = "Mean")) + 
  geom_vline(data = res.2.1$summary.hyperpar, aes(xintercept = mode[4], color = "Mode", fill = "Mode")) + 
...
#configuration 2.1 --> basic LC model with alpha as an effect of x
p.hyperpars.2.1 <- (p.prec.alpha.2.1 | p.prec.beta.2.1) / ( p.prec.kappa.2.1 | p.prec.epsilon.2.1) +
  plot_layout(guides = "collect") &
...
p.prec.beta.2.2 <- ggplot(data.frame(res.2.2$marginals.hyperpar) %>%
                            filter(Precision.for.beta.x < 200)) + 
  geom_area(aes(x = Precision.for.beta.x, y = Precision.for.beta.y, fill = "Mean"), alpha = 0.4) + 
  geom_vline(data = res.2.2$summary.hyperpar, aes(xintercept = mean[2], color = "Mean", fill = "Mean")) + 
  geom_vline(data = res.2.2$summary.hyperpar, aes(xintercept = mode[2], color = "Mode", fill = "Mode")) + 
  geom_vline(aes(xintercept = tau.iid, color = "True value", fill = "True value")) + 
  scale_color_manual(name = " ", values = palette.basis) + 
  scale_fill_manual(name = " ", values = palette.basis) +
  labs(x = "Value of precision", y = " ", title = "Precision for beta")
...
p.prec.epsilon.2.2 <- ggplot(data.frame(res.2.2$marginals.hyperpar) %>%
                               filter(Precision.for.epsilon.x < 60000)) + 
  geom_area(aes(x = Precision.for.epsilon.x, y = Precision.for.epsilon.y, fill = "Mean"), alpha = 0.4) + 
  geom_vline(data = res.2.2$summary.hyperpar, aes(xintercept = mean[4], color = "Mean", fill = "Mean")) + 
  geom_vline(data = res.2.2$summary.hyperpar, aes(xintercept = mode[4], color = "Mode", fill = "Mode")) + 
  geom_vline(aes(xintercept = tau.epsilon, color = "True value", fill = "True value")) + 
  scale_color_manual(name = " ", values = palette.basis) + 
  scale_fill_manual(name = " ", values = palette.basis) +
  labs(x = "Value of precision", y = " ", title = "Precision for epsilon")
...
p.hyperpars.2.2 <- (p.prec.alpha.2.2 | p.prec.beta.2.2) / ( p.prec.kappa.2.2 | p.prec.epsilon.2.2) +
  plot_layout(guides = "collect") &
  plot_annotation(title = "Estimated hyperparameters for Lee-Carter model, with synthetic data")
p.hyperpars.2.2

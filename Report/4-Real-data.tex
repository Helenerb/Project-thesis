...
We begin by comparing prediction results for the univariate LC- and LCC-models to prediction resutls for APC1 and APC2 models. \textcolor{myDarkGreen}{Say something of how we are now able to get a "fair" comparison - using similar priors, the same method of inference? Refer to previous papers on the same topic? }
...
To assess the prediction we use MSE, as well as a mean DSS statistic (MDSS). The DSS statistic is calculated by:
\begin{equation}
    DSS = (\frac{Y_{x,t} - \mu_{x,t}}{\sigma_{x,t}})^2 + 2\cdot \log(\sigma_{x,t}),
\end{equation}
where $Y_{x,t}$ is the observed mortality rate for age group $x$ in year $t$, $\mu_{x,t}$ is the mean of the corresponding predicted mortality rate and $\sigma_{x,t}$ is the standard deviation of the prediction. In addition, we calculated the percentage of the observations that fell within the 95\% confidence bounds of the predictions.

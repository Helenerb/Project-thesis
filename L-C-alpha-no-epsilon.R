...
# removing epsilon from the model to see if inlabru is able to fit the model then
...
eta.func <- function(inter, alpha, beta, phit, kappa){
  return(inter + alpha + beta*phit + beta*kappa)
}
...
form.1 = y ~ Intercept + alpha + beta*phi + beta*kappa

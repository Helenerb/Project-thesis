# Try to test different formulations for the kappa-term to see what kind of results you get then. 
library(INLA)
library(inlabru)
library(ggplot2)
library(patchwork)
...
# with cohort, as you have defined it previously:
...
likelihood.1 = like(formula = form.1, family = "poisson", data = lung.cancer, E = lung.cancer$total.t)
...
gg.alpha <- ggplot(data.alpha, aes(x = id.order)) + 
...
gg.beta <- ggplot(data.beta, aes(x = ID)) + 
  #geom_ribbon(aes(ymin = X0.025quant, ymax = X0.975quant), fill = "lightskyblue1") + 
...
gg.kappa <- ggplot(data.kappa, aes(x = ID)) + 
  #geom_ribbon(aes(ymin = X0.025quant, ymax = X0.975quant), fill = "lightskyblue1") + 
  geom_errorbar(aes(ID, min = `0.025quant`, ymax =`0.975quant`), position=position_dodge(width=0.5)) +
  geom_point(aes(y = mean, color = "Estimated")) + 
  ggtitle("Kappa - real data for lung cancer")

data.gamma = res$summary.random$gamma %>%
  mutate(id.order = factor(ID, levels=ID))
gg.gamma <- ggplot(data.gamma, aes(x = id.order)) + 
  #geom_ribbon(aes(ymin = X0.025quant, ymax = X0.975quant), fill = "lightskyblue1") + 
  geom_errorbar(aes(id.order, min = `0.025quant`, ymax =`0.975quant`), position=position_dodge(width=0.5)) +
  geom_point(aes(y = mean, color = "Estimated")) + 
  ggtitle("Gamma - real data for lung cancer") +
  scale_x_discrete(breaks=seq(-85, 17, 5))
...
ggplot(data = data.eta.t, aes(x = t)) + 
  ...
  ggtitle("Eta for t - real data for lung cancer") + xlab("t") + ylab("Predictor")

data.eta.x <- data.eta %>% 
  group_by(x) %>% summarize(eta.x = mean(eta))
ggplot(data = data.eta.x, aes(x = x)) + 
  geom_point(aes(y = eta.x, color = "Estimated")) + 
  ggtitle("Eta for x - real data for lung cancer") + xlab("t") + ylab("Predictor")



#   ----   new formulation, removing the random-walk component from the formula:   ----


# with cohort, as you have defined it previously:
form.2 = total ~ -1 + Int + alpha + beta*phi + gamma + epsilon

# add data as lung.cancer and offset as population$total, which is the number of people at-risk
likelihood.2 = like(formula = form.2, family = "poisson", data = lung.cancer, E = lung.cancer$total.t)
...
res.2 = bru_rerun(res.2)

res.2$summary.fixed
res.2$summary.hyperpar

data.alpha.2 = res.2$summary.random$alpha %>%
  mutate(id.order = factor(ID, levels=ID))
gg.alpha.2 <- ggplot(data.alpha.2, aes(x = id.order)) + 
  #geom_ribbon(aes(ymin = X0.025quant, ymax = X0.975quant), fill = "lightskyblue1") + 
  geom_errorbar(aes(id.order, min = `0.025quant`, ymax =`0.975quant`), position=position_dodge(width=0.5)) +
  geom_point(aes(y = mean, color = "Estimated")) + 
  ggtitle("Alpha - real data for lung cancer")

data.beta.2 = res.2$summary.random$beta 
gg.beta.2 <- ggplot(data.beta.2, aes(x = ID)) + 
  #geom_ribbon(aes(ymin = X0.025quant, ymax = X0.975quant), fill = "lightskyblue1") + 
  geom_errorbar(aes(ID, min = `0.025quant`, ymax =`0.975quant`), position=position_dodge(width=0.5)) +
  geom_point(aes(y = mean, color = "Estimated")) + 
  ggtitle("Beta - real data for lung cancer")

data.gamma.2 = res.2$summary.random$gamma %>%
  mutate(id.order = factor(ID, levels=ID))
gg.gamma.2 <- ggplot(data.gamma.2, aes(x = id.order)) + 
  #geom_ribbon(aes(ymin = X0.025quant, ymax = X0.975quant), fill = "lightskyblue1") + 
  geom_errorbar(aes(id.order, min = `0.025quant`, ymax =`0.975quant`), position=position_dodge(width=0.5)) +
  geom_point(aes(y = mean, color = "Estimated")) + 
  ggtitle("Gamma - real data for lung cancer") +
...
data.phi.2 = data.frame(cbind(ID = unique(lung.cancer$t), 
                            mean = res.2$summary.fixed$mean[2]*unique(lung.cancer$t),
                            X0.025quant = res.2$summary.fixed$`0.025quant`[2]*unique(lung.cancer$t),
                            X0.975quant = res.2$summary.fixed$`0.975quant`[2]*unique(lung.cancer$t)))
gg.phi.2 <- ggplot(data = data.phi.2, aes(x = ID)) + 
  geom_ribbon(aes(ymin = X0.025quant, ymax = X0.975quant), fill = "lightskyblue1") + 
  geom_point(aes(y = mean, color = "Estimated")) + 
  ggtitle("Phi - real data for lung cancer")

data.eta.2 <- data.frame(eta = res.2$summary.linear.predictor$mean[1:324]) %>%
  mutate(t = lung.cancer$t, x = lung.cancer$x)
gg.eta.2 <- ggplot(data = data.eta, aes(x=t, y=x, fill = eta)) + geom_tile() + 
  xlab("Time: 1999 - 2016") + 
  ylab("Age: 0 - 85+") + 
  ggtitle("")

(gg.alpha.2 | gg.beta.2 | gg.phi.2) / (gg.gamma.2 | gg.eta.2)
